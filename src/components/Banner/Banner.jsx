...
const BannerSection = () => {
  ...
  return (
    <section className="banner-section">
      <OwlCarousel key={city} className="banner-carousel owl-theme" {...owlOptions}>
        {[1, 2].map((slideIndex) => (
            ...
            <div className="auto-container">
              <div className="content-box">
                ...
                <div className="search-bar">
                    ...
                    {showDropdown && (
                      <ul className="cities-dropdown-menu">
                        {locations.map((cityOption, index) => (
                          <li key={index} onClick={() => handleCitySelection(cityOption)} className="city-drop-item">
                            {cityOption}
                          </li>
                        ))}
                      </ul>
                  ...
                  <button className="find-jobs-button" onClick={handleSearch}>Find Jobs</button>
                </div>
              </div>
            </div>
          </div>
